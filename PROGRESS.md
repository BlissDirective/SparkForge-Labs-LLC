...
### FORGE HUB — W2-08 remaining MOTION_BIBLE morphs (Director, 2026-09-15)

Director remainder ids are live GSAP timelines on the existing forge slice (no new Zustand store): `hub-labsbrowse` / `labsbrowse-hub`, `focus-in` / `focus-out`, `dual-enter` / `dual-exit`, `whisper-expand` / `whisper-close`, `lobby-playstage-merge` / `playstage-lobby-split`. Canonical order charge → beams → slabs → fade first fifth → wipe last fifth → Sparky → sting. Interactive ≤ 600 ms; RM = 200 ms crossfade. Slab/`slotSlide` = Stagehand `LAYOUT_MORPH_MS` 420 except `playstage-lobby-split` 340 ms (bible §5.5). HoloC live seat `{31.2, 24, 37.6×48}`. Portal 420/560 stays `emit-burst` only (not nested). `/dev/forge-hub` HUD remainder picker + Stagehand transition scrubber can trigger the ids; emit-burst / login-success / ignition / Skip stay in place for Stagehand W2-07. Theatre `level-up` / `outfit-swap` unregistered. Board: W2-06 done (#181); W2-08 merged #183 `30c1337`. PR #164 stays open.
Agent: Director (Grok Bot Team)

### FORGE HUB — W2-07 HoloC login + P2 morph cycle (Stagehand, 2026-09-15)

Live welcome login form on HoloC (`WelcomeLoginForm` — real email/password, TAP §2.9). Stays mounted through `welcome → hubSplit → playStage → gameLobby → welcome` (`hidden` when not welcome so the node never unmounts). `/dev/forge-hub` submit plays Director `login-success-hubsplit`. Rebased onto #183 `30c1337`. Does not invent gameLobby layout seats (`hubSplit → playStage` stays `applyForgeRoute`; named merge is `gameLobby → playStage`). `playStage → gameLobby` uses live `playstage-lobby-split`. Director HUD remainder picker / scrubber / calibrate / EscapeFlat / ToastRail intact. Portal 420/560, HoloC `{31.2,24,37.6×48}`, `LAYOUT_MORPH_MS` 420 locked. No production `FORGE_HUB` flag. Merged PR #182 `3022ccb`.
Agent: Stagehand (Grok Bot Team)

### FORGE HUB — W2-09 PR #164 close audit (Stagehand, 2026-09-15)

Audited Scribe’s `docs/forge-hub/PR164_PORT_LIST.md` (PR #170, never merged to tip) against `3022ccb`. W1/W2 PORT rows were on tip except **catalog copy** — landed here as `src/lib/forge-hub/catalog.ts` (`HOLO_C_WELCOME`, HoloBubble tips, 11-lab rows). Do-not-port items confirmed absent (`forge-lab` tree, `/dev/forge-lab`, `FORGE_LAB_HUB`, live `authMerged` / `TopBanner` / `heroWelcome`). GitHub had already closed #164 at 05:09:19Z (with W2-05 merge) without Scribe’s comment and before W2-07; W2-09 posts the official not-merged close comment. Next: W2-10 `createRenderer` / WebGPU (not started here).
Agent: Stagehand (Grok Bot Team)
