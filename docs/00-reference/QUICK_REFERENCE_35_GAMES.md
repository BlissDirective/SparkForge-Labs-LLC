# SparkForge — Quick Reference: All 35 Games

**Extracted from:** CLAUDE.md v5.6 Section 13
**Date:** March 19, 2026
**Canonical source:** GCUD V10.2

| # | Game | Lab | Slug | Tier | 3D | Bands | Stage |
|---|------|-----|------|------|-----|-------|-------|
| 1 | AI Spy | 1 | ai-spy | Std | Env | A,B,C | 7A |
| 2 | Time Machine | 1 | time-machine | Std | Env | A,B,C | 7A |
| 3 | Human vs Machine | 1 | human-vs-machine | Std | Env | A,B,C | 7B |
| 4 | AI Pet Trainer | 2 | pet-trainer | Flag | Full | A,B,C | 6B |
| 5 | Sort Toy Box | 2 | sort-toy-box | Full3D | Full | A,B,C | 7B |
| 6 | Treat Trainer | 2 | treat-trainer | Std | Env | A,B,C | 7C |
| 7 | Data Detective | 2 | data-detective | FL-L | Enh | A,B,C | 7C |
| 8 | Neural Builder | 3 | neural-builder | Flag | Full | A,B,C | 6C |
| 9 | Neuron Relay | 3 | neuron-relay | Std | Env | A,B,C | 7C |
| 10 | Pixel Investigator | 3 | pixel-investigator | Std | Env | B,C | 7D |
| 11 | Prompt Lab | 4 | prompt-lab | Flag | Full | A,B,C | 6D |
| 12 | Word Predictor | 4 | word-predictor | Std | Env | A,B,C | 7A |
| 13 | Token Chopper | 4 | token-chopper | Std | Env | B,C | 7A |
| 14 | AI Art Detective | 4 | ai-art-detective | Std | Env | A,B,C | 7A |
| 15 | Agent Architect | 5 | agent-architect | Flag | Full | A,B,C | 6E |
| 16 | Robot Vacuum | 5 | robot-vacuum | FL-L | Enh | A,B,C | 7D |
| 17 | Tool Picker | 6 | tool-picker | Std | Env | A,B,C | 7A |
| 18 | Bias Detective | 6 | bias-detective | Flag | Full | B,C | 6F |
| 19 | Data Shield | 6 | data-shield | Std | Env | A,B,C | 7A |
| 20 | Real or Fake | 6 | real-or-fake | Std | Env | A,B,C | 7A |
| 21 | Ethics Courtroom | 6 | ethics-courtroom | Std | Env | B,C | 7E |
| 22 | Camera Quest | 7 | camera-quest | FL-L | Enh | A,B,C | 7D |
| 23 | Fool the AI | 7 | fool-the-ai | Std | Env | B,C | 7D |
| 24 | Build Classifier | 7 | build-classifier | Std | Env | B,C | 7E |
| 25 | Prediction Market | 7 | prediction-market | Std | Env | B,C | 7A |
| 26 | Sentiment Scanner | 8 | sentiment-scanner | Std | Env | A,B,C | 7C |
| 27 | Chatbot Builder | 8 | chatbot-builder | FL-L | Enh | B,C | 7C |
| 28 | Lost in Translation | 8 | lost-in-translation | Std | Env | A,B,C | 7C |
| 29 | Emoji Decoder | 8 | emoji-decoder | FL-L | Enh | A,B | 7F |
| 30 | Code Blocks | 9 | code-blocks | FL-L | Enh | A,B,C | 7B |
| 31 | Career Explorer | 9 | career-explorer | Std | Env | B,C | 7B |
| 32 | API Explorer | 9 | api-explorer | Std | Env | C | 7E |
| 33 | My First AI App | 9 | my-first-ai-app | FL-L | Enh | A,B,C | 7F |
| 34 | Future Forge | 10 | future-forge | FL-L | Enh | A,B,C | 7D |
| 35 | AI or Not? | 10 | ai-or-not | FL-L | Enh | A,B | 7F |

---

**Tier Legend:** Flag = Flagship (Full 3D, 20M), Full3D = Full 3D (20M), FL-L = FL-Lite (Immersive 3D, 10M), Std = Standard (Immersive 3D, 5M)
**3D Legend:** Full = Full R3F 3D scene, Enh = Enhanced R3F environment, Env = Dedicated 3D environment (5M budget)
**Band Legend:** A = Ages 7-9, B = Ages 10-12, C = Ages 13-16
