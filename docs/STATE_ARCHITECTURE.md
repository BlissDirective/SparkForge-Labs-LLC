...
| `sceneStore` (`useForgeStore` alias) | — | — | 49 | Active 3D scene selection, transition state, game HUD content. **W1-01:** Forge Hub slice. **W1-02:** `dispatchForgePortal` runs the ported `idle→charge→emit→docked` reducer on `forge.portalPhase` (same store — TAP §2.5). |
