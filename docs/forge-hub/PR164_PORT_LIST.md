...
**Status:** LOCKED inventory for Stagehand (W1/W2) and Gatekeeper  
**Date:** 2026-09-15  
**Task:** W0-07 (Scribe)  
**Rule:** **Port, then close.** Do **not** close [PR #164](https://github.com/BlissDirective/SparkForge-Labs-LLC/pull/164) until the W2 port is complete.
...
## Stay-open rule (Gatekeeper / Scribe)

PR **#164 stays open** until Stagehand has ported the **PORT** table into `/dev/forge-hub` (W1 reducer + W2 layout registry, catalog, tests, blend tokens) and those tests are green on `setup-sparkforge-dev`.

Then **Gatekeeper + Scribe** close #164 with a comment pointing at:

1. TAP v2.2 decision 9 (“Port, then close.”)
2. This file (`docs/forge-hub/PR164_PORT_LIST.md`)
3. The W2 PR(s) that landed the port

Closing **now** (after this list only) is a process bug. Do not squash-merge #164 onto `setup-sparkforge-dev` either — the hotspot shell, `/dev/forge-lab` route, and `FORGE_LAB_HUB` flag must never land.

Comment draft for that later close is at the bottom of this file. **Do not post it until W2 is done.**
...
|---|---|---|
| **W1** Stagehand | `portalMachine.ts` + portal unit test; plate aspect / `FORGE_CORE` for the TSL emitter; blend **edge** tokens on glass slabs; locked plate already in `public/forge-hub/world/` as backdrop + SSIM + poster. | P1: `/dev/forge-hub` room shell, reducer `idle→charge→emit→docked`, SSIM ≥ 0.96. **#164 still open.** |
| **W2** Stagehand | `layouts.ts` → world-unit registry; catalog copy; slot/yaw tests; `HoloPanel` reading plate (fill ≥ 0.85); beam attach retarget; `/dev/forge-hub` mode switcher. | P2: login form on HoloC through `welcome → hubSplit → playStage → gameLobby → welcome`; ported tests green. **Then close #164.** |
...
## Comment draft for PR #164 (post after W2 — do not post now)

Use once the W2 port PR is on `setup-sparkforge-dev` and the PORT tests are green. Gatekeeper posts; Scribe may co-sign. Point at TAP + this list.
...
Agent: Gatekeeper (Grok Bot Team), with Scribe
```

Attribution line for the GitHub close (GROK_TEAM_PROMPTS §9): add the Gatekeeper / Scribe call signs as above. Do not merge. **Close as not merged.**
