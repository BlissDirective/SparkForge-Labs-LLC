...
| P1 | Stagehand → owner | P1 gate packet | **SSIM passes (0.969 WebGL2 vs LOCKED_HUB.jpg); WebGPU number pending O-3** | Room shell re-tuned to the canonical plate 2026-09-15. Live-mode registry re-tuned to the equal trio 2026-09-19 (W1-05 done). Remaining before the packet: WebGPU SSIM on the reference laptop (O-3). |
| W1-05 | Stagehand (reviewer covering) | Re-tune the live-mode layout registry to the equal-trio plate | **done** | 2026-09-19: `layouts.ts` re-read off `LOCKED_HUB.jpg`. `HUBSPLIT_HOLO_C` {37,13.5,26,49}, `labsBrowse`, `focus`, `dual` (two panels + wide chart band lifted off the emitter), `avatarStudio` all re-seated into the painted glass band (top ≥13.5 %, bottom ≤62.5 %) so they clear the desk edge (~70 %) and the bottom-centre emitter disc (FORGE_CORE cy 78). `PLAYSTAGE_CENTER` → tall spec-literal {15,13.5,70,73} per owner call (2026-09-19) — the merged slab englobes the disc, so `liveBeams` draws no external cone when merged. Overlay verified on the plate (all resting modes clear the disc; only playStage covers it). Lock pose unchanged, so SSIM is unaffected. Tests updated (guards + director lerp targets). Owner P1/P4 visual sign-off still pending. |
