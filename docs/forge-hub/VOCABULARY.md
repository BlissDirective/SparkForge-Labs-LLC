...
| `ToastRail` | Edge chips: offline, demo, verify, toasts. (Decision 14, 2026-09-15: the site footer — copyright, cookie preferences, COPPA and no-tracking badges, legal links — lives **inside a side hologram** (`HoloR` footer strip in `welcome`, `HoloL` in `hubSplit`), not on the rail. There is no marketing or hero page outside the forge.) |
