...
| **z2** | Real React UI: `ForgeCore` (live SF), `TopMonitor` HUD fitted 1:1 in the bezel glass (yaw 0), `HoloPanel`s that emit from SF. |
...
Idle and docked share one plate. Empty cyan panes on the plate are **guides only** (no fake UI). HTML `HoloPanel` owns the **entire** hologram — cyan edge, frosted fill, glow, and content — as **one** `perspective(1200px) rotateY(±3deg)` element.

Do **not** paint thick yawed slabs into the art and overlay a second untransformed HTML box (hybrid dock). Keep plate panes faint/empty so they do not fight the HTML chrome.

Locked regen (brighter orange floor/cowling lights, metal darkness unchanged) is used for `00-locked-hub-dark-sf.png`, `01-idle.png`, and `04-docked.png`.
...
|---|---|---|
| `TOP_MONITOR_GLASS` | `27.4, 2.6, 45.2 × 17.8` | `0` |
...
- Click SF / pedestal → charge (420ms) → emit (560ms) → dock panels left/right and light the top bay.
