...
> **NOTE (April 3, 2026):** The celebration and particle components (CeremonyFX, GameParticles3D, ParticlePreferenceControl) described here have been augmented by the 3D UI Migration (Phase 4) with XPPopup3D and CelebrationPanel3D. The existing components remain valid and are orchestrated by the new `useCelebration3D` hook in the celebration pipeline.
