...
export function RealOrFakeGame() {
  ...
  return (
    <GameShell gameId="real-or-fake" title="Real or Fake?" worldNumber={6} worldColor="#EF4444">
                ...
                {phase === 'welcome' && (
                  <motion.div key="welcome" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <motion.button onClick={() => setPhase('play')}
                      ...
                      Start Detecting! <Fingerprint className="inline w-4 h-4 ml-1" />
                    </motion.button>
                  </motion.div>
                )}
                  ...
                  <motion.div key="play" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-md space-y-2">
                    ...
                    <div className="flex gap-3 mb-4 justify-center">
                      ...
                      <motion.button onClick={() => handleGuess(true)} disabled={!!feedback}
                        className="px-8 py-3 rounded-xl bg-red-500/15 border border-red-500/30 font-display text-sm font-bold text-red-400 flex items-center gap-2"
                    ...
                    <AnimatePresence>
                      {feedback && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                          className={`rounded-xl p-3 max-w-sm mx-auto ${feedback.correct ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-red-500/10 border border-red-500/20'}`}>
