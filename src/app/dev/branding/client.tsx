...
function PrimitiveFor({ subject }: { subject: Exclude<Subject, 'sf-mark'> }) {
  switch (subject) {
    case 'cube':
...
export function BrandingDevClient() {
  const [subject, setSubject] = useState<Subject>('sf-mark');
  ...
  return (
    <main className="min-h-screen bg-[#02050d] text-white/90">
      <div className="mx-auto max-w-[1400px] px-6 py-8">
        ...
        {/* Live tuning sliders */}
        <div className="mb-4 grid grid-cols-1 gap-3 rounded-lg border border-white/10 bg-white/[0.02] p-3 sm:grid-cols-3">
          <Slider
            label="Dispersion ×"
            value={dispersionMul}
            min={0}
        ...
        <div className={`grid gap-4 ${showRef ? 'lg:grid-cols-2' : 'grid-cols-1'}`}>
          <section className="relative aspect-[4/3] overflow-hidden rounded-xl border border-white/10 bg-[#02050d]">
            <BrandingShowcase
              cameraDistance={subject === 'sf-mark' ? 6.4 : 4.4}
        ...
        <footer className="mt-8 space-y-2 text-xs text-white/40">
          ...
          <p>
            SF geometry source: <code>public/branding/sf-geometry.svg</code> —
            single source of truth. Edit and refresh to iterate; SSIM gate
            converges at ≥ 0.96 vs reference.
          </p>
        </footer>
      </div>
    </main>
  ...
  return (
    <label className="flex flex-col gap-1.5 text-xs text-white/70">
      <div className="flex items-center justify-between">
        <span>{label}</span>
        <span className="tabular-nums text-white/90">{value.toFixed(3)}</span>
      </div>
      <input
        type="range"
        min={min}
