// ════════════════════════════════════════════════════════════════
// Forge Hub catalog — PR #164 catalog.ts, re-seated (W2-09)
// ════════════════════════════════════════════════════════════════
// Port: HubStats, LabRow, LAB_POETIC (11 labs), Lucide LAB_ICONS,
// buildLabRows, welcome copy, PREVIEW_STATS / PREVIEW_PROGRESS,
// sparkyLine, xpDialValue, pickContinueLab.
//
// Rename: FORGE_LAB_WELCOME → HOLO_C_WELCOME (HoloC, TAP §2.9).
// sparkyLine strings are HoloBubble `tip` copy, not a TopMonitor HUD.
// Glazier later swaps production login copy onto HoloC.
...
/** Lucide only — no emoji (no-emoji ratchet). Canonical names stay in labs.ts. */
...
/** Welcome copy on HoloC (decision 7 / TAP §2.9). */
export const HOLO_C_WELCOME = {
  title: 'Welcome to SparkForge',
  subtitle:
    'Where kids learn AI by building, playing, and exploring with Sparky.',
  rotating: [
...
/** HoloBubble `tip` strings (text only in v1). Not a TopMonitor HUD. */
export const HOLO_BUBBLE_TIPS = [
  ...
  'Streaks live on HoloC. I am the voice in the bubble.',
  'Outfits come later. Your face still belongs to you.',
  'Games play on PlayStage. This bay is only a mount.',
] as const;

export function sparkyLine(seed: number): string {
  return HOLO_BUBBLE_TIPS[Math.abs(seed) % HOLO_BUBBLE_TIPS.length];
}
