...
export const lab3Pattern = Fn(() => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    totalPulse = add(totalPulse, add(ring, mul(glowPulse, float(0.1))));
  }

  // Connection lines between nodes (simplified: 4 connections)
  let connections = float(0.0);
  for (let i = 0; i < 4; i++) {
    const fi = float(i);
    const fi1 = float(i + 1);
    ...
    connections = add(connections, mul(mul(smoothstep(float(0.008), float(0.002), lineDist), pulse), float(0.3)));
  }

  totalPulse = add(totalPulse, connections);
