// ================================================================
// SparkForge Shared GLSL Noise Library
// ================================================================
// Used by: aurora, labPatterns, liquidMetal, energyField, fireNoise
// Import via: src/shaders/index.ts concatenation
...
vec2 curlNoise(vec2 p) {
  float eps = 0.001;
  float n1 = simplex2D(vec2(p.x, p.y + eps));
  float n2 = simplex2D(vec2(p.x, p.y - eps));
  float n3 = simplex2D(vec2(p.x + eps, p.y));
  float n4 = simplex2D(vec2(p.x - eps, p.y));
